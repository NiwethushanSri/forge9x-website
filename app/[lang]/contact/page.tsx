...
import { Phone, Clock, Send, Paperclip } from "lucide-react";
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
              ...
              {[
                ...
                <div key={item.label} className="bg-[#0a0a0a] rounded-2xl p-6 border border-gray-900">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center text-blue-400 bg-blue-500/10">{item.icon}</div>
                    <div className="font-semibold text-white">{item.label}</div>
                  </div>
                  {item.lines.map((line) => <div key={line} className="text-sm text-gray-400 ml-12">{line}</div>)}
                </div>
              ))}
            </div>
            ...
            <div className="lg:col-span-2 bg-[#0a0a0a] rounded-2xl p-8 border border-gray-900">
              {sent ? (
                ...
                <>
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">{t.badge}</p>
                  <h2 className="text-3xl md:text-4xl font-bold text-white mb-4 leading-tight">{t.title}</h2>
                  <p className="text-gray-400 text-base leading-relaxed mb-8 max-w-lg">{t.desc}</p>
                  <div className="border-t border-gray-800 mb-8" />
                  <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-1.5">{t.nameLabel} *</label>
                      <input required type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder={t.namePlaceholder} className="w-full px-4 py-2.5 rounded-xl border border-gray-700 text-sm focus:outline-none focus:border-blue-500 bg-gray-800 text-white placeholder-gray-500" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-1.5">{t.emailLabel} *</label>
                      <input required type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder={t.emailPlaceholder} className="w-full px-4 py-2.5 rounded-xl border border-gray-700 text-sm focus:outline-none focus:border-blue-500 bg-gray-800 text-white placeholder-gray-500" />
                    </div>
                    <div>
                    ...
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-300 mb-1.5">{t.messageLabel} *</label>
                      <textarea required rows={5} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder={t.messagePlaceholder} className="w-full px-4 py-2.5 rounded-xl border border-gray-700 text-sm focus:outline-none focus:border-blue-500 resize-none bg-gray-800 text-white placeholder-gray-500" />
                    </div>
                    <div className="md:col-span-2">
                    ...
                    <div className="md:col-span-2 overflow-x-auto">
                      <ReCAPTCHA ref={recaptchaRef} sitekey="6LdnYEotAAAAAIlZiscunlw-QHKl3VS1l9M4VRBi" onChange={(token) => setCaptchaToken(token)} onExpired={() => setCaptchaToken(null)} theme="dark" />
                    </div>
                    <div className="md:col-span-2">
                      <button type="submit" disabled={!agreed || !captchaToken} className="px-8 py-3 rounded-full font-semibold text-white transition-all hover:brightness-110 flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed" style={{ backgroundColor: "#00679A" }}>
